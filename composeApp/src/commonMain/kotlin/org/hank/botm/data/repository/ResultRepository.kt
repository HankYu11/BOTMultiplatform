...
interface ResultRepository {
    ...
    fun getRoundResults(roundId: Long): Flow<List<Result>>
    suspend fun insertResults(results: List<Result>, roundId: Long)
}
...
class ResultRepositoryImpl (
    ...
    override fun getRoundResults(roundId: Long): Flow<List<Result>> =
        resultDao.getRoundResults(roundId).map {
            it.map { it.asDomain() }
        }


    override suspend fun insertResults(results: List<Result>, roundId: Long) = withContext(ioDispatcher) {
